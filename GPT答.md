...
“方程组是强刚性”这个术语通常出现在数学和工程学中，特别是在描述某些类型的线性或非线性方程组时。强刚性是指方程组解的敏感性，即对输入或参数变化的反应程度。

具体来说，强刚性有两个常见的解释：

###  **解的稳定性**：

在某些情况下，如果方程组是“强刚性”的，意味着它的解对初始条件或输入的微小变化非常敏感。换句话说，方程组的解对某些参数或变量的扰动有很强的依赖性，即使是微小的变化也可能导致解的显著变化。这在数值分析中尤其重要，特别是在处理不稳定或容易引发数值误差的系统时。

###  **解的存在性和唯一性**：

在一些应用中，强刚性可能指的是方程组解的存在性或唯一性受非常严格的条件约束。例如，某些非线性方程组在特定条件下可能只会有一个解，并且解的行为对参数变化极为敏感。这种类型的系统也可能表现出强刚性。

### **应用于结构分析**：

在工程学中，尤其是涉及结构力学时，强刚性指的是系统的刚度很大。比如在结构分析中，强刚性的结构意味着在外力作用下，结构的变形非常小。这种结构需要非常大的外力才能导致显著的变形，反映了它的刚度（或抗变形能力）较高。

# **FDS采用LES大涡模拟的动机与网格适配原理**

FDS选择大涡模拟（LES）作为核心湍流建模方法，其设计逻辑与火灾动力学的特殊需求紧密相关。以下从技术选型依据、网格尺度矛盾化解两个层面展开分析：

## **一、FDS采用LES的核心原因**

1. **捕捉瞬态湍流-火焰相互作用**
    火灾燃烧本质为高度非稳态的湍流反应流，火焰前锋的褶皱、卷吸现象由大尺度涡主导。LES通过直接求解大尺度涡（>网格尺寸）的运动，同时模型化小尺度涡（<网格尺寸）的能量耗散，可精确刻画火焰动态传播、烟气分层等关键过程，而传统RANS（雷诺平均）方法因时间平均会抹除这些瞬态细节。
2. **平衡精度与计算成本**
    相较于直接数值模拟（DNS）需解析所有湍流尺度（Kolmogorov尺度，毫米级以下），LES仅需解析积分尺度（厘米至米级），显著降低计算量。对于建筑火灾等大空间场景，LES能在有限算力下提供工程可接受的精度。
3. **匹配火灾物理特征**
    火灾中热浮力驱动流占主导，湍流脉动能量集中在低频大涡（如热羽流中的旋涡结构）。LES对大涡的显式解析更符合此类流动的频谱特性，而小尺度涡对整体燃烧速率的影响可通过亚格子（Subgrid-Scale, SGS）模型合理近似。

## **二、FDS如何用厘米级网格实现有效LES模拟**

尽管经典LES理论要求网格尺寸（Δ）小于湍流积分尺度（l），但FDS通过以下策略在厘米级网格下保持计算合理性：

1. **物理问题简化与模型适配**
   - **低马赫数假设**：火灾流动马赫数通常<0.3，FDS忽略可压缩性效应，避免声波尺度（微秒级）的时间步长限制，允许使用较大时间步长（0.01~1秒）。
   - **混合燃烧模型**：默认的混合分数燃烧模型将化学反应简化为混合过程，减少对湍流-化学相互作用（需极细网格）的直接依赖。
2. **动态亚格子模型优化**
   - **Smagorinsky-Lilly模型修正**：FDS通过降低Smagorinsky常数（默认值0.2→0.1~0.15），减少对网格分辨率的敏感度，允许更大Δ下仍能合理预测湍动能级联。
   - **显式滤波函数**：通过空间滤波分离大/小尺度涡，即使Δ较粗，仍能保证SGS模型仅作用于未被解析的高频脉动。
3. **网格分级与局部加密**
   - **多重网格域（Multi-Block）**：在火焰区、通风口等关键区域局部加密网格至毫米级，其他区域保持厘米级，整体计算量可控（如图1所示）。
   - **自适应网格（试验阶段）**：部分版本支持动态调整网格密度，追踪火焰锋面移动。
4. **经验验证与标定**
    FDS通过大量全尺寸实验（如NIST的PRISME系列火灾测试）校准模型参数，确保厘米级网格下的预测结果（如热释放率、烟气温度）与实测误差<15%，从而在工程精度内接受“粗网格LES”的近似性。